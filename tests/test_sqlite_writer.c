...
TEST(sw_stream_open_does_not_truncate_destination) {
    ...
    ASSERT(fp != NULL);
    ASSERT_EQ(fwrite(sentinel, 1, sizeof(sentinel) - 1, fp), sizeof(sentinel) - 1);
...
TEST(sw_publish_removes_destination_sidecars) {
    ...
    ASSERT(fp != NULL);
    ASSERT_EQ(fwrite("stale-wal", 1, 9, fp), 9);
    ASSERT_EQ(fclose(fp), 0);

    fp = fopen(shm, "wb");
    ASSERT(fp != NULL);
    ASSERT_EQ(fwrite("stale-shm", 1, 9, fp), 9);
    ASSERT_EQ(fclose(fp), 0);

    int rc = cbm_write_db(path, "test", "/tmp/test", "2026-07-07T00:00:00Z", NULL, 0, NULL, 0, NULL,
                          0, NULL, 0);
    ASSERT_EQ(rc, 0);
    ASSERT(access(wal, F_OK) != 0);
...
TEST(sw_publish_failure_removes_temp_output) {
    char path[256];
